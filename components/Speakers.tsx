    ...
    speakers: [
      {
        ...
        bio: venueOrganisers.filter((o) => o.name === "Daniel Huckfield")[0].bio + "" + venueOrganisers.filter((o) => o.name === "Daniel Huckfield")[0].bio2 + " " + venueOrganisers.filter((o) => o.name === "Daniel Huckfield")[0].bio3,
      },
    ],
  ...
  },
  {
    ...
    speakers: [
      ...
      },
      {
        ...
        affiliation: "The Game Centre, Manchester Metropolitan University",
        bio: mainOrganisers.find(mo => mo.name === "Dr Chloé Germaine")?.bio || "" + mainOrganisers.find(mo => mo.name === "Dr Chloé Germaine")?.bio2 || "",
      },
    ],
    ...
    speakers: [
      {
        ...
        bio: mainOrganisers.find(mo => mo.name === "Dr Gray Atherton & Dr Liam Cross")?.bio || "" + mainOrganisers.find(mo => mo.name === "Dr Gray Atherton & Dr Liam Cross")?.bio2 || "" ,
      },
    ],
...
const TYPE_CONFIG: Record<Session["type"], { label: string; icon: string; colour: string; bg: string; border: string }> = {
  talk:       { label: "Talk",       icon: "🎤", colour: "#29abe2", bg: "rgba(41,171,226,0.10)",  border: "rgba(41,171,226,0.25)" },
  workshop:   { label: "Workshop",   icon: "🛠️", colour: "#e8731a", bg: "rgba(232,115,26,0.10)", border: "rgba(232,115,26,0.28)" },
  panel: { label: "Panel", icon: "💬", colour: "#6a5a40", bg: "rgba(106,90,64,0.10)",  border: "rgba(106,90,64,0.28)" },
};
...
function SpeakerBio({ speaker }: { speaker: Speaker} ) {
  ...
  return (
    <div className={`w-fit ${speaker.bio ? "col-span-2 -order-1" : ""}`}>
      ...
      <p className="font-body text-xs" style={{ color: "#7a6a50" }}>{speaker.affiliation}</p>
      {open && (
        <p
...
function SessionCard({ session, index }: { session: Session; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-wrap gap-2">
          ...
          <span
            ...
            style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, color: cfg.colour }}
          >
            <span>{cfg.icon}</span>
            {cfg.label}
          </span>
        ...
        {session.duration && (
          <span className="font-body text-xs shrink-0" style={{ color: "#a89878" }}>
            {session.duration}
          </span>
        )}
      </div>
      ...
      {/* Speakers */}
      {session.speakers && (
        <div className={`${session.speakers.length > 4 ? "grid grid-cols-2" : "flex flex-col"} gap-3`}>
      ...
      {session.abstract && (
        <div>
          ...
          <svg
            ...
            style={{ transform: abstractOpen ? "rotate(90deg)" : "rotate(0deg)" }}
            fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"
          >
        ...
        {abstractOpen && (
          <p
            ...
            style={{ color: "#4a4030", borderTop: "1px solid rgba(0,0,0,0.07)" }}
          >
            {session.abstract}
          </p>
        )}
    ...
    <section id="speakers" ref={ref} className="py-24 px-6 topo-bg" style={{ background: "#f5efe0" }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
          ...
          <p className="reveal reveal-delay-2 font-body text-sm max-w-md leading-relaxed" style={{ color: "#5a4a30" }}>
          ...
          {filters.map(({ key, label, icon }) => {
            ...
            return (
              <button
                ...
                className="inline-flex items-center gap-1.5 font-body text-sm font-semibold px-4 py-2 rounded-full transition-all duration-200"
                style={{
                  background: active ? "#29abe2" : "rgba(255,255,255,0.7)",
                  color: active ? "white" : "#5a4a30",
                  border: active ? "1.5px solid #29abe2" : "1.5px solid rgba(0,0,0,0.10)",
                  boxShadow: active ? "0 2px 8px rgba(41,171,226,0.25)" : "none",
                }}
              >
                <span>{icon}</span>
                {label}
                <span
                  ...
                  style={{
                    background: active ? "rgba(255,255,255,0.25)" : "rgba(41,171,226,0.12)",
                    color: active ? "white" : "#29abe2",
                  }}
                >
                  {count}
                </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {filtered.map((session, i) => (
            <SessionCard key={`${session.type}-${session.title}`} session={session} index={i} />
          ))}
        </div>
